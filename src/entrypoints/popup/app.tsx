...
import {PayrollCode, ScheduledShift, ScheduledShiftsData} from "@/entrypoints/content/api/scheduled-shifts-response.ts";
...
function buildSubtreeForMonth(month: string, events?: EventAttributes[]) {
  return !events || events.length === 0 ? undefined : {
    id: month,
    label: month,
...
function convertToEventAttribute(schedule: ScheduledShiftsData, shift: ScheduledShift): EventAttributes {
  const department = schedule.region_departments
      .find((department) => department.id === shift.department);
  const occupation = schedule.occupations
      .find((occupation) => occupation.id === shift.occupation);
  return {
...
function isWorkingShift(payroll_codes: PayrollCode[], shift: ScheduledShift) {
  const payrollCode = payroll_codes
      .find((code) => code.id === shift.payroll_code);
