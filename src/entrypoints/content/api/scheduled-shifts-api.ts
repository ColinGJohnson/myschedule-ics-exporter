...
export async function fetchScheduledShifts(employeeId: number, year: number, monthIndex: number): Promise<ApiResponse> {
  try {
    ...
    console.error(`Error fetching shifts for employeeId=${employeeId} year=${year} monthIndex=${monthIndex}`, error);
    return { error: (error instanceof Error) ? error.message : "A network error occurred." }
  }
}
